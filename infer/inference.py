...
def main(args):
    ...
    out_file_prefix = f'{args.test_id}_{args.split}_{file_prompt_type}_{args.num_test_sample}_seed{args.seed}_t{args.temperature}'
    ...
    if len(examples) == 0:
        processed_samples = processed_samples[args.start:args.end]
    ...
    for example in tqdm(examples, total=len(examples)):
        ...
        # add remain fields
        for key in ['level', 'type', 'unit', 'solution_type', 'choices', 'solution', 'ques_type', 'ans_type','queId']:
            if key in example:
    ...
    for epoch in range(max_func_call):
        ...
        # print('len(remain_prompts):',len(remain_prompts))
        ...
        elif len(outputs) < len(current_prompts):
            print('len(outputs) <  len(current_prompts)')
            outputs += ['0' for i in range(len(current_prompts) - len(outputs))]
        else:
            pass
        ...
        for (i, query), output in zip(current_prompts, outputs):
            # print('zip_query:',query)
            # print('zip_output:',output)
        ...
        for k in range(len(remain_prompts)):
            # print('len(remain_prompts):',len(remain_prompts))
            # print('remain_prompts:', remain_prompts)
