...
def extract_answer_from_gsm_hard(pred_str):
    preds = []
    if 'boxed' in pred_str:
        ...
        pred = a
        preds.append(pred)
        preds.append(extract_program_output(pred_str))
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    elif data_name in ["TAL-EN",'TAL-SCQ-CN']:
        if "Answer: $$" in example['solution']:
            gt_cot = 0
            gt_ans = example['solution'].split("Answer: $$")[-1]
            gt_ans = gt_ans.split("$$")[0]
        else:
            gt_cot = 0
            gt_ans = example['solution'].split("Answer:")[-1]
...
def run_execute(executor, result, prompt_type, execute=False,gsm_hard=False):
    ...
    if "program_only" in prompt_type:
        ...
        if not gsm_hard:
            ...
            prediction = extract_answer_from_gsm_hard(result)
